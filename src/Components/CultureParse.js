...
const CultureParse = () => {
  ...
  const handleSentenceChange = (newSentences) => {
    ...
    const [startParen, endParen] = [
      nextSentence.indexOf("("),
      nextSentence.indexOf(")"),
    ];
    let maybeNumber;
  ...
  const prettyPrint = () => {
    ...
    const prettyData = sortedData.map(([key, count]) => {
      const firstWord = key.includes(" ")
        ? key.slice(0, key.indexOf(" "))
        : key;
      const rest =
        key.includes(" ") && ` ${key.slice(key.indexOf(" ")).trim()}`;
      return (
        <>
  ...
  const totalEntries = () =>
    Object.values(keyData).reduce((acc, cur) => acc + cur, 0);
  ...
  const newEntry = () => {
    setKeyData({
      ...keyData,
      [trimNumbers(textToSave).trim()]: occurencesFromString,
    });
    advanceCurrent();
  };
    ...
    <div className="App">
      ...
      <div>
        <input
          type="number"
          style={{ width: 49 }}
          value={occurencesFromString}
          onChange={(event) =>
            setOccurencesFromString(parseInt(event.target.value, 10))
          }
        />
      </div>
      <div>
        ...
        <button
          disabled={Object.keys(keyData).includes(trimNumbers(textToSave))}
          onClick={newEntry}
        >
          New Entry
        </button>
      ...
      <div>
        {Object.entries(keyData)
          ...
          .map(([key, count]) => (
            <div>
              {count}: <button onClick={() => incrementKey(key)}>{key}</button>
            </div>
          ))}
      </div>
      &nbsp;
      ...
      <GetSampleData setData={updateInput} />
    </div>
  );
};
